...
# read ML trees 
seg_A_capsid.tree <- read.nexus(here("local/paktrees/original", tree_files[6]))
seg_Asia1_capsid.tree <- read.nexus(here("local/paktrees/original", tree_files[7]))
seg_O_capsid.tree <- read.nexus(here("local/paktrees/original", tree_files[9]))
...
ggplot(rf_df, aes(Var1, Var2, fill = Freq)) +
  ...
  scale_fill_gradientn(
    colors = rev(pals::ocean.speed(100)[1:70]) 
  ) +
  theme_classic() +
  theme(
...
# to sf Object
farms_sf <- st_as_sf(farm_data, coords = c("coord_y", "coord_x"), crs = 4326)
...
map_plot <- ggmap(background_map) +
  geom_sf(data = farms_sf_jitter %>% filter(!has_sample), 
          fill = "gray50", color = "gray50", size = 3, shape = 21, inherit.aes = FALSE) +
  geom_sf(data = farms_sf_jitter %>% filter(has_sample), 
...
farms_sf_unique <- farms_sf_jitter %>%
...
map_plot <- ggmap(background_map) +
  geom_jitter(
    data = farms_sf_unique,
    aes(x = x, y = y, fill = farm_code),
    size = 5, shape = 21, alpha = 0.7, color = "transparent", 
